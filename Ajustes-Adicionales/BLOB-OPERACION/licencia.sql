CREATE OR REPLACE PROCEDURE carga_img_licencia(
    empleado_id_insert IN NUMBER
)
IS
    v_bfile            BFILE;
    v_blob             BLOB;
...
BEGIN
    ...
    INTO v_blob
    FROM empleado
    WHERE empleado_id = empleado_id_insert
    FOR UPDATE;
...
CREATE OR REPLACE TRIGGER trg_licencia_post_insert_img
AFTER INSERT ON empleado
FOR EACH ROW
BEGIN
    carga_img_licencia(:NEW.empleado_id);
END;
/
SHOW ERRORS
